...
def drawScales(self, qp, startTime=time.time(), drawTime = time.time(), endCount=0, xRatio=1):
    ...
    if endCount>HolterWidth*0.83/xstep*240/xRatio:
        ...
        deltaCount = HolterWidth*(0.83)/xstep*240/xRatio
        deltaTime = deltaCount/4
        startTime = drawTime - deltaTime
    else:
        pass
    if xRatio == 1:
        seconds_per_scale = 60
        seconds_per_bigscale = seconds_per_scale*10
        pix_per_bigscale = xstep * 10
    elif xRatio == 2:
        seconds_per_scale = 30
        seconds_per_bigscale = seconds_per_scale*20
        pix_per_bigscale = xstep * 20
    else:
        seconds_per_scale = 20
        seconds_per_bigscale = seconds_per_scale*30
        pix_per_bigscale = xstep * 30
    #rest of second before first 10Min scale
    restTime=seconds_per_bigscale-float(startTime)%seconds_per_bigscale
    #rest of second before first 1Min scale   
    restSecond=restTime%seconds_per_scale
    #pix of first column
    halfstep=round(restSecond*xstep/seconds_per_scale)
    #pix of first big scale
    halfBig=round(restTime*xstep/seconds_per_scale)
    #max complete column in whole Holter
    xstepMax=int((HolterWidth-halfstep)/xstep)+1
    #max complete big column in whole Holter
    xBigMax=int((HolterWidth-halfBig)/pix_per_bigscale)+1
    #time stamp of first 10Min scale
    firstScale = startTime+restTime    
    #print 'restTime:', restTime, 'restSecond:', restSecond, 'halfstep:', halfstep, 'halfBig:', halfBig, 'firstScale', firstScale   
    '''
    FHR scale below
    '''
    for i in range(17):
    ...
    Time scale below
    '''
    pen = QtGui.QPen(QtGui.QColor(165, 173, 181), 2, QtCore.Qt.SolidLine)
    qp.setPen(pen)
    qp.setFont(QtGui.QFont('Decorative', 10))
    for i in range(xBigMax):
        qp.drawLine(round(pix_per_bigscale*i+halfBig), 0, round(pix_per_bigscale*i+halfBig), HolterHeight)
        timeScale = time.strftime('%H:%M:%S', time.localtime(firstScale+i*seconds_per_bigscale))
        qp.drawText(pix_per_bigscale*i+halfBig,ystepHR*32-10,timeScale )

...
#                self.childWidget_list[i].update_paint(self.PCalList[i])    
...
#        self.label_blueTooth.setGeometry(QtCore.QRect(self.size().width()-120,10, 32,32))
    ...
    def on_pushSearch_clicked(self):
        ...
        Slot documentation goes here.
        """
        uuid_Dict = self.request_patient()
        if uuid_Dict:
    ...
    def request_patient(self):
        ...
        if uuid_Dict:
            for key in uuid_Dict:
                self.an24Dict[key] = UiData(key, uuid_Dict[key])
                self.an24List.append(key)  
...
#        print 'widget prepaired', time.time()
#        self.allCache = OpenCsv(path)
#        print 'read over', time.time()
#        for i in range(len(self.an24Dict)):
#            self.an24Dict[self.an24List[i]].start_time=time.time()
#            self.an24Dict[self.an24List[i]].data_handler.data_recv(self.allCache)
#        print 'threading over', time.time()
        #真实流程
...
if __name__ == "__main__":
    ...
    uuid_Dict = ist.request_patient()
    print 'uuid_Dict:', uuid_Dict
    if uuid_Dict:
        ist.start_detecting()
    else:
        print 'no patient online'
    #ist.an24Dict = DlgLogin.lgn('localhost', '123', ist.Connection)    
    sys.exit(app.exec_())

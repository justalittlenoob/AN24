...
def drawXScale(self, qp,   window_rect, unitXY, all_count, start_count = 0, end_count = 0,   xRatio = 1, startTime = time.time()) :
...
def drawNote(self, qp, FHR_unitXY, all_count,  start_count,  end_count, xRatio,  noteDict) :
    pen_solid= QtGui.QPen(QtGui.QColor(165, 173, 181), 1, QtCore.Qt.SolidLine)
    qp.setFont(QtGui.QFont('Decorative', 10))
    unitX, unitY = FHR_unitXY
    for key in noteDict:
        if key>start_count:
            note_x = unitX * (key-start_count)
            note_y = noteDict[key][0]
            note = noteDict[key][1]
...
def realtime_monitor(self, qp, Cache, start_count, end_count, scaleY,  unitXY, pen):
    cachePre=[]
    i=0

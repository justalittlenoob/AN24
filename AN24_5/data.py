...
def data_parse(cblock_str, run_chk, low_battry):
    ...
    if(len(cblock_str) == CBLOCK_STR_LEN):
        ...
        MHR = [0, 0, 0, 0]
        mother_mv = [0, 0, 0, 0]
        init_An24.run_check(cblock_str, run_chk)
...
def data_recv_An24(sock, data_cache, run_chk, low_battry):
    # ************
...
import threading
def start_data_thread(sock, data_cache, run_chk):
    '''Make a thread'''
    
    threading.Thread(target=data_recv_An24,
            args=(sock,data_cache,run_chk,low_battry)
        ).start() 

...
#            realEvent(self, qp, Cache, start_count, end_count, self.unitXY['FHR'], self.line_rgb['Event'])       
    ...
    def search_an24(self, AN24_nameDict):
        self.an24Chosen = DlgSearch.choose_AN24(AN24_nameDict)
        if self.an24Chosen:
            nameChosen = self.an24Chosen['name']
    ...
    def listen_alarm(self):
        try:        
            ...
            try:
                ...
                if i>=230 and self.an24Dict[nameChosen].alarmed_count == -1:
                    self.is_alarm = True
                if self.is_alarm:
                    self.sound_off()
                ...
                if self.an24Dict[nameChosen].alarmed_count >=0:
                    self.an24Dict[nameChosen].alarmed_count += 8
                    if self.an24Dict[nameChosen].alarmed_count >= 480:
                        self.an24Dict[nameChosen].alarmed_count = -1
    @pyqtSignature("bool")    
    ...
    def on_pushButtonAlarm_clicked(self):
        ...
        try:        
            ...
            self.an24Dict[self.an24Chosen['name']].alarmed_count= 0
        self.sound_on()
